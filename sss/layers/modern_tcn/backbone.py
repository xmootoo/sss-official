...
import math
from sss.layers.modern_tcn.utils import series_decomp, Flatten_Head, RevIN

# Taken from: https://github.com/luodhhh/ModernTCN/tree/main/ModernTCN-classification/models
...
class Stage(nn.Module):
    def __init__(self, ffn_ratio, num_enc_layers, large_size, small_size, dmodel, dw_model, nvars,
...
class ModernTCNBackbone(nn.Module):
    def __init__(self, task_name, patch_dim, patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_enc_layers, large_size, small_size, dims, dw_dims,
                 nvars, small_kernel_merged=False, backbone_dropout=0.1, head_dropout=0.1, use_multi_scale=True, revin=True, affine=True,
                 subtract_last=False, seq_len=512, c_in=7, individual=False, target_window=96, class_drop=0., num_classes = 10, return_head=True):
        ...
        stem = nn.Sequential(
            nn.Conv1d(1, dims[0], kernel_size=patch_dim, stride=patch_stride),
            nn.BatchNorm1d(dims[0])
        )
        self.downsample_layers.append(stem)

        self.num_stage = len(num_enc_layers)
        if self.num_stage > 1:
        ...
        for stage_idx in range(self.num_stage):
            layer = Stage(ffn_ratio, num_enc_layers[stage_idx], large_size[stage_idx], small_size[stage_idx], dmodel=dims[stage_idx],
        ...
        if self.task_name == 'classification':
            ...
            self.head_in_dim = self.n_vars*self.head_nf
            self.head_class = nn.Linear(self.n_vars*self.head_nf,self.num_classes)
    ...
    def forward_feature(self, x):
        ...
        for i in range(self.num_stage):
            ...
            x = x.reshape(B * M, D, N)
            if i==0:
                if self.patch_dim != self.patch_stride:
                    # stem layer padding
                    pad_len = self.patch_dim - self.patch_stride
