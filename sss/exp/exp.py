...
class Experiment:
    ...
    def run(self, rank=0, world_size=1):
        ...
        if self.args.exp.learning_type == "ssl":
            if not self.args.downstream.ignore_pretrain: self.pretrain()
            self.downstream()
    ...
    def init_ddp(self, rank, world_size):
        ...
        self.sync()
        self.print_master(f"Initialized Distributed Data Parallel aross {self.world_size} devices.")
    ...
    def init_dataloaders(self, learning_type="sl", loader_type="train"):
        ...
        if self.args.exp.sklearn:
            ...
            self.print_master(f"{len(self.train_loader)} train samples. {len(self.val_loader)} validation samples. {len(self.test_loader)} test samples.")
            return
    ...
    def seq_load(self, loader_type="train", learning_type="sl"):
        self.console.log(f"Running sequential dataloading on rank {self.rank} ({loader_type}).")
        self.free_memory()
        loaders = get_loaders(self.args, learning_type, self.generator, self.rank, self.world_size, self.args.sl.dataset_class, loader_type)
    ...
    def init_model(self):
        ...
        if self.args.exp.sklearn:
            self.print_master(f"Scikit-learn model {self.args.exp.model_id} initialized.")
            return
        elif self.args.ddp.ddp:
            torch.cuda.set_device(self.rank)
            self.model.to(self.rank)
            self.model = DDP(self.model, device_ids=[self.rank], find_unused_parameters=self.args.ddp.find_unused_parameters)
        else:
    ...
    def init_earlystopping(self, path : str):
        self.early_stopping = EarlyStopping(patience=self.args.early_stopping.patience,
    ...
    def sklearn_eval(self, model, criterion, inputs, targets, acc=False, mae=False, ch_acc=False, flag="sl"):
        stats = dict()
        ...
        if self.args.open_neuro.task=="binary":
            probs = probs[:, 1] # Get positive class value. Otherwise use multiclass values
        ...
        if self.args.exp.other_metrics:
            if self.args.open_neuro.task == "binary":
                single_metrics = binary_classification_metrics(preds, targets, probs, self.device) # Shape: (7,)
            elif self.args.open_neuro.task == "multi":
                single_metrics = multi_classification_metrics(preds, targets, probs, self.device) # Shape: (15,)
            else:
                raise ValueError("Invalid task.")
        elif acc:
            single_metrics = ((preds == targets).float().mean()).unsqueeze(0) # Shape: (1,)
        else:
            single_metrics = None
        ...
        if acc or ch_acc:
            ...
            update_stats(stats, single_metrics, self.args.open_neuro.task, self.args.exp.other_metrics, ch_acc)
    ...
    def train(self, model, model_id, optimizer, train_loader, best_model_path, criterion, val_loader=None, scheduler=None, flag="sl", ema=None, mae=False, acc=False, early_stopping=False, ch_acc=False):
        """
            Trains a model.
        ...
        for epoch in range(eval(f"self.args.{flag}.epochs")):
            ...
            for i, batch in enumerate(train_loader):
                ...
                if (i+1) % 100 == 0:
                    ...
                    self.print_rank(f"Train Loss before all_reduce: {running_loss}. Rank: {self.rank}")
                    self.print_rank(f"Num examples before all_reduce: {running_num_examples}. Rank: {self.rank}")
                    ...
                    if self.args.ddp.ddp:
                        ...
                        self.print_rank(f"Train Loss after all_reduce: {loss_tensor.item()}. Rank: {self.rank}")
                        self.print_rank(f"Num examples after all_reduce: {num_examples_tensor.item()}. Rank {self.rank}")
                    ...
                    if self.rank == 0:
                        ...
                        self.print_master(f"EMA decay rate: {alpha}") if flag=="ssl" else None
                ...
                if scheduler:
                    if self.args.exp.model_id == "PatchTSTOG":
                        self.sync()
                        new_lr = adjust_learning_rate(optimizer, scheduler, epoch, self.args)
                        self.print_master(f"Learning rate adjusted to: {new_lr}") if ((i+1) % 100 == 0 or i == len(train_loader) - 1) else None
                    self.sync()
                    scheduler.step()
            ...
            # <--------------- Validation --------------->
            if val_loader:
                self.validate(model, val_loader, model_id, criterion, flag, mae, acc, epoch, best_model_path, early_stopping, ch_acc)
            ...
            if early_stopping:
                if self.rank == 0 and self.early_stopping.early_stop:
                    ...
                    if self.args.ddp.ddp:
                        should_stop_tensor = torch.tensor(1, dtype=torch.int, device=self.device)
                        dist.broadcast(should_stop_tensor, src=0)
                    break
                elif self.args.ddp.ddp:
                    # On non-master GPUs, receive the broadcast to stop
                    should_stop_tensor = torch.tensor(0, dtype=torch.int, device=self.device)
                    dist.broadcast(should_stop_tensor, src=0)
    ...
    def validate(self, model, val_loader, model_id, criterion, flag, mae, acc, epoch, best_model_path, early_stopping, ch_acc):
        """
            Validate the model.
        """
            ...
            if early_stopping:
                ...
                if val_metric < self.best_val_metric:
                    if self.args.exp.best_model_metric=="loss":
                        ...
                        self.print_master(f"Validation {self.args.exp.best_model_metric} increased ({-self.best_val_metric*100:.3f}% --> {-val_metric*100:.3f}%).")
    ...
    def pretrain(self):
        ...
        self.ssl_scheduler = get_scheduler(self.args, self.args.ssl.scheduler, "pretrain", self.optimizer, len(self.train_loader))

        # SSL criterion
        self.ssl_criterion = get_criterion(self.args, self.args.ssl.criterion)

        if self.args.ema.ema_momentum_scheduler:
            self.ema = ema_momentum_scheduler(len(self.train_loader),
    ...
    def downstream(self):
        ...
        self.downstream_criterion = get_criterion(self.args, self.args.downstream.criterion)

        # Downstream Optimizer
        self.downstream_optimizer = get_downstream_optim(self.args, self.downstream_model, self.args.downstream.eval, self.args.downstream.optimizer)

        # Downstream Scheduler
        self.downstream_scheduler = get_scheduler(self.args, self.args.downstream.scheduler, "downstream", self.downstream_optimizer, len(self.downstream_train_loader))

        # Downstream Training
        self.print_master("Starting Downstream Training...")
        best_model_path = os.path.join(self.log_dir, "downstream.pth")
        self.train(model=self.downstream_model,
        ...
        self.test(model=self.downstream_model,
                  model_id=self.args.downstream.model_id,
                  best_model_path=best_model_path,
                  criterion=self.downstream_criterion,
                  flag="downstream",
    ...
    def test(self, model, model_id, best_model_path, criterion, flag="sl", mae=False, acc=False, ch_acc=False):
        ...
        #<---Scikit-learn pipeline--->
        if self.args.exp.sklearn:
            self.sklearn_eval(model, criterion, self.test_loader.x, self.test_loader.y, acc, mae, ch_acc, flag)
            return
        ...
        self.train_calibrator(model, model_id, criterion, flag) # Optional: Train probability calibrator before evaluation
        ...
        if self.rank == 0:
            ...
            acc = False if self.args.data.full_channels else acc
            self.log_stats(stats, flag, mae, acc, ch_acc, mode="test")
            self.tuning_score = stats[f"{self.args.exp.tuning_metric}"] # For hyperparameter tuning

    def evaluate(self, model: nn.Module, model_id: str, loader: DataLoader, criterion: nn.Module, flag: str, mae=False, acc=False, ch_acc=False, calibrate=False):
        """
        ...
        if self.args.ddp.ddp:
            self.sync()
            self.print_rank(f"Evaluation loss before all_reduce: {total_loss.item()}. Rank: {self.rank}")
            dist.all_reduce(total_loss, op=dist.ReduceOp.SUM)
        if self.rank == 0: stats["loss"] = total_loss.item() / num_examples
        self.sync()
        self.print_rank(f"Evaluation loss after all_reduce: {total_loss.item()}. Rank: {self.rank}.")
        ...
        if mae:
            ...
            self.sync()
            if self.rank == 0: stats["mae"] = total_mae.item() / num_examples

        # Window Metrics
        if acc:
            window_metrics = get_metrics(self.args, all_logits, all_labels, mode="window", rank=self.rank, calibrator=self.calibrator)
            channel = True if self.args.data.full_channels else False
            update_stats(stats, window_metrics, self.args.open_neuro.task, self.args.exp.other_metrics, channel, self.rank)

        # Channel Metrics
        if ch_acc:
            ch_metrics = get_metrics(self.args, all_logits, all_labels, all_ch_ids, all_u, mode="channel", rank=self.rank, calibrator=self.calibrator)
            update_stats(stats, ch_metrics, self.args.open_neuro.task, self.args.exp.other_metrics, True, self.rank)
    ...
    def train_calibrator(self, model, model_id, criterion, flag="sl"):
        ...
        if self.args.exp.calibrate:
            ...
            probs, targets, ch_ids = self.calibrator.compile_predictions(all_probs, all_targets, all_ch_ids)
    ...
    def log_stats(self, stats, flag, mae, acc, ch_acc, mode, task="binary"):
        ...
        if ch_acc:
            ...
            epoch_logger(self.args, self.logger, f"{flag}_{mode}/channel_accuracy", ch_acc_value)
